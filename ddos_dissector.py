...
def get_repository(args, config):
    ...
    if not (args.host):
        ...
        logger.critical(
            "Cannot find repository {} credentials. You should define in the cmd line or configuration file [dddosdb.conf].".format(
                args.host))
        return None
...
def clusterization_multifrag(df_filtered, n_type):
    ...
    try:
        frag_percentage = \
            df_[(df_['fragmentation'] > SIMILARITY_THRESHOLD) & (df_['index'].values)[0] == True].values[0][1]
...
if __name__ == '__main__':
    ...
    (enriched_fingerprint, json_file) = prepare_fingerprint_upload(df_filtered, df, fingerprints, n_type, labels,
